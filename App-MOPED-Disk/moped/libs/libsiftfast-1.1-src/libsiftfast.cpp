...
// if defined will align all SFImage rows to 16 bytes
...
typedef struct ImageSt {
    int rows, cols;          // Dimensions of SFImage.
    float *pixels;          // 2D array of SFImage pixels.
...
extern "C" {
SFImage CreateImage(int rows, int cols);
SFImage CreateImageFromMatlabData(double* pdata, int rows, int cols);
void DestroyAllImages();
Keypoint GetKeypoints(SFImage porgimage);
SFImage SiftDoubleSize(SFImage p);
SFImage SiftCopyImage(SFImage p);
SFImage HalfImageSize(SFImage curimage);
Keypoint OctaveKeypoints(SFImage pimage, SFImage* phalfimage, float fscale, Keypoint prevkeypts);
void SubtractImage(SFImage imgdst, SFImage img0, SFImage img1);
void GaussianBlur(SFImage imgdst, SFImage SFImage, float fblur);
void ConvHorizontal(SFImage imgdst, SFImage SFImage, float* kernel, int ksize);
void ConvHorizontalFast(SFImage imgdst, SFImage SFImage, float* kernel, int ksize);
void ConvVertical(SFImage SFImage, float* kernel, int ksize);
void ConvVerticalFast(SFImage SFImage, float* kernel, int ksize);
void ConvBuffer(float* buf, float* kernel, int cols, int ksize);
Keypoint FindMaxMin(SFImage* imdiff, SFImage* imgaus, float fscale, Keypoint prevkeypts);
void GradOriImages(SFImage imgaus, SFImage imgrad, SFImage imorient);
void GradOriImagesFast(SFImage imgaus, SFImage imgrad, SFImage imorient);
int LocalMaxMin(float fval, SFImage imdiff, int row, int col);
int NotOnEdge(SFImage imdiff, int row, int col);
Keypoint InterpKeyPoint(SFImage* imdiff, int index, int rowstart, int colstart,
                        SFImage imgrad, SFImage imorient, char* pMaxMinArray, float fscale,Keypoint keypts, int steps);
float FitQuadratic(float* X, SFImage* imdiff, int index, int rowstart, int colstart);
void SolveLinearSystem(float* Y, float* H, int dim);
Keypoint AssignOriHist(SFImage imgrad, SFImage imorient, float fscale, float fSize,
...
Keypoint MakeKeypoint(SFImage imgrad, SFImage imorient, float fscale, float fSize,
                      float frowstart,float fcolstart,float forient, Keypoint keypts);
void MakeKeypointSample(Keypoint pnewkeypt, SFImage imgrad, SFImage imorient,
                        float fSize, float frowstart, float fcolstart);
void NormalizeVec(float* pf, int num);
void KeySample(float* fdesc, Keypoint pnewkeypt, SFImage imgrad, SFImage imorient,
               float fSize, float frowstart, float fcolstart);
void AddSample(float* fdesc, Keypoint pkeypt, SFImage imgrad, SFImage imorient, int r, int c,
...
static list<SFImage> s_listImages;
SFImage CreateImage(int rows, int cols)
{
    SFImage im;

    im = (SFImage) sift_aligned_malloc(sizeof(struct ImageSt),16);
    im->rows = rows;
    im->cols = cols;
...
void DestroyAllImages()
{
    for(list<SFImage>::iterator it = s_listImages.begin(); it != s_listImages.end(); ++it) {
...
SFImage CreateImageFromMatlabData(double* pdata, int rows, int cols)
{
    SFImage SFImage = CreateImage(rows,cols);
    float* pixels = SFImage->pixels;
    int stride = SFImage->stride;
...
static SFImage* s_imgaus = NULL, *s_imdiff = NULL;
static SFImage s_imgrad = NULL, s_imorient = NULL;
...
Keypoint GetKeypoints(SFImage porgimage)
{
...

    {
        ...
        s_imgaus = new SFImage[((27 + 4*Scales)&0xfffffff0)/4];
        s_imdiff = new SFImage[((23 + 4*Scales)&0xfffffff0)/4];
...
SFImage SiftDoubleSize(SFImage im)
{
    int rows = im->rows, cols = im->cols;
    int newrows = 2*rows-2, newcols = 2*cols-2;
    SFImage newim = CreateImage(newrows, newcols);
...
SFImage SiftCopyImage(SFImage im)
{
    DVSTARTPROFILE();
    SFImage newim = CreateImage(im->rows,im->cols);
...
SFImage HalfImageSize(SFImage curimage)
{
    ...
    SFImage halfimage = CreateImage(newrows, newcols);
...
#ifdef __SSE__
    if( SFImage->cols < 12 )
        ConvHorizontal(imgdst, SFImage,kernel,ksize);
    else
        ConvHorizontalFast(imgdst, SFImage,kernel,ksize);
...
void ConvHorizontal(SFImage imgdst, SFImage SFImage, float* kernel, int ksize)
{
    ...
    _buf.resize(SFImage->cols + ksize);
    float* buf = &_buf[0];
    int rows = SFImage->rows, cols = SFImage->cols, stride = SFImage->stride;
    int width = (ksize >= 0 ? ksize : ksize-1)>>1;
    float* pixels = SFImage->pixels;
...
void ConvVertical(SFImage SFImage, float* kernel, int ksize)
{
    ...
    static vector<float> _buf; //TODO, make 16 byte aligned
    _buf.resize(SFImage->rows + ksize);
    float* buf = &_buf[0];
    int rows = SFImage->rows, cols = SFImage->cols, stride = SFImage->stride;
    int width = (ksize >= 0 ? ksize : ksize-1)>>1;
    float* pixels = SFImage->pixels;
...
#ifdef ALIGNED_IMAGE_ROWS
    assert( !(SFImage->stride&3) );
#endif
    
    DVSTARTPROFILE();
    
    int width = (ksize >= 0 ? ksize : ksize-1)>>1;
    float* _pixels = SFImage->pixels, *_pdst = imgdst->pixels;
...
void ConvVerticalFast(SFImage SFImage, float* kernel, int ksize)
{
    int rows = SFImage->rows, stride = SFImage->stride;
    ...
    int convsize = max(100000,32*(SFImage->rows + ksize));
...
Keypoint FindMaxMin(SFImage* imdiff, SFImage* imgaus, float fscale, Keypoint keypts)
{
    DVSTARTPROFILE();
...
void GradOriImages(SFImage image, SFImage imgrad, SFImage imorient)
{
    DVSTARTPROFILE();
...
#if !defined(_MSC_VER) && defined(__SSE__)
void GradOriImagesFast(SFImage image, SFImage imgrad, SFImage imorient)
{
    DVSTARTPROFILE();
...
int LocalMaxMin(float fval, SFImage imdiff, int rowstart, int colstart)
{
...
float FitQuadratic(float* X, SFImage* imdiff, int index, int r, int c)
{
    float H[9];
...
void MakeKeypointSample(Keypoint pkeypt, SFImage imgrad, SFImage imorient,
                        float fSize, float frowstart, float fcolstart)
{
...
void KeySample(float* fdesc, Keypoint pkeypt, SFImage imgrad, SFImage imorient,
               float fSize, float frowstart, float fcolstart)
{
    int rowstart = (int)(frowstart+0.5f);
    int colstart = (int)(fcolstart+0.5f);
